...
// WhatsApp's LID rollout means Baileys can hand back a participant as a plain
// jid string OR as an object (e.g. { id, jid, lid }) depending on version/context.
// Coerce defensively so mention-sending never crashes on the object shape.
function participantJid(p) {
  if (typeof p === 'string') return p;
  if (p && typeof p === 'object') return p.jid || p.id || p.lid || '';
  return '';
}
...
// File-based storage: kept only as a fallback for local dev when no DATABASE_URL is set.
// On Render, disks are ephemeral, so this alone does NOT survive restarts/redeploys.
...
function saveConfigFile() { fs.writeFileSync(configFile, JSON.stringify(config, null, 2)); }

// Primary storage: Postgres, so settings survive restarts/redeploys on Render.
async function loadConfigFromPostgres() {
  const result = await pool.query('SELECT jid, config FROM bot_group_config');
  const byJid = new Map(result.rows.map(r => [normalizeJid(r.jid), makeGroupConfig(r.jid, r.config)]));
  for (const jid of GROUP_JIDS) if (!byJid.has(jid)) byJid.set(jid, makeGroupConfig(jid));
  return { groups: [...byJid.values()] };
}
async function saveConfigToPostgres() {
  for (const group of config.groups) {
    await pool.query(
      `INSERT INTO bot_group_config(jid, config, updated_at) VALUES($1,$2,NOW())
       ON CONFLICT(jid) DO UPDATE SET config=EXCLUDED.config, updated_at=NOW()`,
      [group.jid, JSON.stringify(group)]
    );
  }
}
...
async function loadConfig() {
  if (dbReady) {
    try { return await loadConfigFromPostgres(); }
    catch (e) { logger.warn({ err: e }, 'Failed to load config from Postgres, falling back to file'); }
  }
  return loadConfigFile();
}
async function saveConfig() {
  if (dbReady) {
    try { await saveConfigToPostgres(); return; }
    catch (e) { logger.warn({ err: e }, 'Failed to save config to Postgres, falling back to file'); }
  }
  saveConfigFile();
}
function getGroupConfig(jid) {
  ...
  if (!group) {
    ...
    saveConfig().catch(e => logger.warn({ err: e }, 'Failed to persist new group config'));
  }
  return group;
}

// Safe synchronous default until the real config is loaded (from Postgres, once ready) in app.listen below.
let config = { groups: GROUP_JIDS.map(jid => makeGroupConfig(jid)) };
let sock = null;
...
async function connectWhatsApp() {
  ...
  sock.ev.on('group-participants.update', async ({ id, participants, action }) => {
    ...
    const group = getGroupConfig(jid);
    // Coerce each participant to a plain jid string up front (see participantJid),
    // and skip any we can't resolve instead of letting one bad entry crash the batch.
    const jids = participants.map(participantJid).filter(Boolean);
    try {
      if (action === 'add' && group.welcomeEnabled) {
        for (const p of jids) {
          try { await sock.sendMessage(jid, { text: mentionsFor(group.welcomeText, p), mentions: [p] }); }
          catch (e) { logger.warn({ err: e, jid, p }, 'Welcome message failed for participant'); }
        }
      }
      if (action === 'add' && group.greetEnabled) {
        for (const p of jids) {
          try { await sock.sendMessage(jid, { text: mentionsFor(group.greetText, p), mentions: [p] }); }
          catch (e) { logger.warn({ err: e, jid, p }, 'Greeting failed for participant'); }
        }
      }
      if (action === 'promote' && group.welcomeNewAdmins) {
        for (const p of jids) {
          try { await sock.sendMessage(jid, { text: mentionsFor(group.newAdminText, p), mentions: [p] }); }
          catch (e) { logger.warn({ err: e, jid, p }, 'New-admin message failed for participant'); }
        }
      }
